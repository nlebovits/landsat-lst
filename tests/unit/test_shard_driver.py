...
class TestClusterNames:
    ...
    def test_each_round_gets_its_own_name(self, storage, plan, job, fast_barriers):
        ...
        assert climatology == sorted(set(climatology))
        assert len(climatology) == 2
        assert climatology[0].endswith("-r1")
        assert climatology[1].endswith("-r2")
...
class TestInFlightAdoption:
    ...
    def test_a_fresh_submission_record_is_adopted_rather_than_resubmitted(
        ...
        publish_plan(storage, plan)
        record_in_flight(storage, plan, "climatology", [0, 1])

        watching = LandsOnPoll(storage, plan, "climatology", after=3)
        fleet = FakeFleet(storage, plan)
        summary = resume_tile(RUN_ID, TILE, storage=watching, submit=fleet)

        assert "climatology" not in fleet.stages
        assert summary.completed
        climatology = next(s for s in summary.stages if s.stage == "climatology")
        assert climatology.submissions == 0
        assert climatology.adopted == 1
    ...
    def test_a_second_driver_submits_nothing_while_round_one_is_fresh(
        ...
        monkeypatch.setattr(settings, "shard_driver_poll_s", 0.001)
        publish_plan(storage, plan)
        record_in_flight(storage, plan, "climatology", [0, 1], age_s=5.0)

        watching = LandsOnPoll(storage, plan, "climatology", after=4)
        fleet = FakeFleet(storage, plan)
        resume_tile(RUN_ID, TILE, storage=watching, submit=fleet)

        assert [stage for stage, _ in fleet.calls if stage == "climatology"] == []
    ...
    def test_after_the_deadline_only_the_missing_indexes_go_again(
        ...
        publish_plan(storage, plan)
        seed = FakeFleet(storage, plan)
        seed(stage="climatology", run_id=RUN_ID, tile=TILE, indexes=[0])  # round 1 landed one
        record_in_flight(storage, plan, "climatology", [0, 1], age_s=10_000.0)

        fleet = FakeFleet(storage, plan)
        summary = resume_tile(RUN_ID, TILE, storage=storage, submit=fleet)

        assert [indexes for stage, indexes in fleet.calls if stage == "climatology"] == [[1]]
        assert fleet.names[fleet.stages.index("climatology")].endswith("-r2")
        assert summary.completed
    ...
    def test_the_round_budget_is_the_stage_not_the_driver(self, storage, plan, fast_barriers):
        ...
        for round_no in (1, 2):
            record_in_flight(
                storage, plan, "climatology", [0, 1], round_no=round_no, age_s=10_000.0
            )

        fleet = FakeFleet(storage, plan)
        with pytest.raises(ShardStageFailed, match="climatology"):
